...
function parseTable(rootNode) {
    ...
    for (let r = 0; r < rows; r++) {
        ...
        for (let c = 0; c < cols; c++) {
            ...
            const node = kind === "row-first" ? containers[r].children[c] : containers[c].children[r];
            cells[r][c] = node;
        }
    }
...
function computeConsistentSizes(cells, rows, cols) {
    const colWidths = [];
    const rowHeights = [];
    for (let c = 0; c < cols; c++) {
        let w = 0;
        for (let r = 0; r < rows; r++) {
            ...
            if (r === 0)
                w = typeof v === "number" ? v : 0;
            else if (typeof v === "number" && v > w)
                w = v;
        }
        colWidths.push(w);
    }
    for (let r = 0; r < rows; r++) {
        ...
        for (let c = 0; c < cols; c++) {
            ...
            if (c === 0)
                h = typeof v === "number" ? v : 0;
            else if (typeof v === "number" && v > h)
                h = v;
        }
        rowHeights.push(h);
    }
...
function setCellLayout(node, layoutAlign, layoutGrow) {
    const n = node;
    if ("layoutAlign" in n)
        n.layoutAlign = layoutAlign;
    if ("layoutGrow" in n)
        n.layoutGrow = layoutGrow;
}
function getLayoutProps(node) {
    ...
    return {
        itemSpacing: typeof base.itemSpacing === "number" ? base.itemSpacing : 0,
        paddingLeft: typeof base.paddingLeft === "number" ? base.paddingLeft : 0,
        paddingRight: typeof base.paddingRight === "number" ? base.paddingRight : 0,
        paddingTop: typeof base.paddingTop === "number" ? base.paddingTop : 0,
        paddingBottom: typeof base.paddingBottom === "number" ? base.paddingBottom : 0,
    };
}
function applyResize(node, width, height) {
...
function createContainerFrame(name, layoutMode, sourceProps) {
...
function switchLayout(parse) {
    ...
    const newKind = kind === "column-first" ? "row-first" : "column-first";
    ...
    const newContainerShouldFill = [];
    if (newKind === "row-first") {
        for (let r = 0; r < rows; r++) {
            newContainerShouldFill[r] = false;
            for (let c = 0; c < cols; c++) {
                if (getCellLayout(cells[r][c]).layoutGrow === 1) {
                    newContainerShouldFill[r] = true;
                    break;
                }
            }
        }
    ...
    const cachedWidths = cells.map(row => row.map(cell => ("width" in cell ? cell.width : 0)));
    const cachedHeights = cells.map(row => row.map(cell => ("height" in cell ? cell.height : 0)));
    ...
    if (newKind === "row-first") {
        for (let r = 0; r < rows; r++) {
            ...
            if (rowCells.every(cell => cell.visible === false)) {
                newContainers[r].visible = false;
        ...
        for (let c = 0; c < cols; c++) {
            if (cells.every(row => row[c].visible === false)) {
                newContainers[c].visible = false;
                for (let r = 0; r < rows; r++)
    ...
    if (newKind === "row-first") {
        for (let r = 0; r < rows; r++) {
            ...
            if (newContainerShouldFill[r]) {
                rowFrame.layoutGrow = 1; // fills root HEIGHT
            }
            else {
        ...
        for (let c = 0; c < cols; c++) {
            ...
            if (newContainerShouldFill[c]) {
                colFrame.layoutGrow = 1; // fills root WIDTH
            }
            else {
